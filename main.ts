...
async function processQueue() {
  ...
  try {
    ...
    resolve(
      new Response(`Proxy error: ${error.message}`, {
        status: 502,
...
async function handler(req: Request): Promise<Response> {
  ...
  if (apiKeys.length === 0) {
    return new Response("Server configuration error: No API keys configured.", {
      status: 500,
      headers: CORS_HEADERS,
    });
  }
  ...
  try {
    ...
    return new Response(`Invalid JSON body: ${error.message}`, {
      status: 400,
      headers: CORS_HEADERS,
    });
  }
